...
def test_enter_on_first_row_selects_first_vehicle() -> None:
    scene = ShopScene()
    game = new_game(1)
    assert scene.commands([_key(pygame.K_RETURN)], game, 1 / 60) == [SelectVehicle("compact")]


def test_cursor_down_reaches_first_item_row() -> None:
    scene = ShopScene()
    game = new_game(1)
    scene.commands([_key(pygame.K_DOWN)] * 4, game, 1 / 60)  # past the 4 vehicles
    assert scene.commands([_key(pygame.K_RETURN)], game, 1 / 60) == [BuyItem("detector")]
...
def test_cursor_up_wraps_to_last_row() -> None:
    ...
    assert scene.commands([_key(pygame.K_RETURN)], game, 1 / 60) == [BuyItem("vacuum")]


def test_f_emits_finish_shopping() -> None:
    scene = ShopScene()
    game = new_game(1)
    assert scene.commands([_key(pygame.K_f)], game, 1 / 60) == [FinishShopping()]


def test_other_keys_emit_nothing() -> None:
    scene = ShopScene()
    game = new_game(1)
    assert scene.commands([_key(pygame.K_SPACE)], game, 1 / 60) == []
